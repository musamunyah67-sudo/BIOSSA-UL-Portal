...
function doPost(e) {
  try {
    ...
    // Call the function with arguments and return its raw result.
    // IMPORTANT: do not wrap this in {success:true, data:result} — the
    // frontend was written against native google.script.run semantics,
    // where withSuccessHandler receives exactly what the server function
    // returned (e.g. res.success, res.token, res.message read directly
    // off the top-level object). Wrapping it here breaks every handler.
