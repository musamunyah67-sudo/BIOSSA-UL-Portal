```javascript
// Vercel serverless function — FAQ/support chatbot for BIOSSA-UL applicants
// and members. Calls Google Gemini (free tier) with a fixed system prompt.
//
// Requires one environment variable in Vercel:
//   GEMINI_API_KEY — from https://aistudio.google.com/apikey
//
// Deliberately does NOT have access to member data, session tokens, or
// Apps Script at all — it only knows the static FAQ text below plus
// whatever the frontend explicitly sends it. Keep it that way; don't
// wire this up to portalGetMyProfile, portalMyTasks, etc.
...
// ---------------------------------------------------------------------
// BIOSSA-UL FAQ / SUPPORT KNOWLEDGE
// ---------------------------------------------------------------------
const FAQ_CONTEXT = `
...
module.exports = async (req, res) => {
  ...
  // Optional, from the widget's own in-memory conversation.
  // Trimmed defensively.
  ...
  try {
    ...
    try {
      ...
      return res.status(502).json({
        error: 'Chat service returned an unexpected response.',
      });
    }
